...
v4l2_init:
	...
	for (i = 0; ; i++)
	{
		...
		if(0 < serial_input_var.DDWS_switch)
		{
			...
			if(g_frame_size > IMAGE_SIZE_FOR_YUVY)
			{
				g_frame_size = IMAGE_SIZE_FOR_YUVY;
			}

			pthread_mutex_lock(&uyvy_image_mutex);

			/* if enter production test mode, get image from local file */
			if(1 == rs485_test_flag)
			{
				if(read_test_picture(YUYV_image, &g_frame_size) < 0)
				{
					memset(YUYV_image, 0, sizeof(YUYV_image));
					g_frame_size = IMAGE_SIZE_FOR_YUVY;
				}
			}
			else  /* if enter normal working mode, get image from camera */
			{
		...
		else  // if DDWS function is disabled, camera is should be closed
		{
			if(0 == close_camera)
			{
				close_camera = 1;
