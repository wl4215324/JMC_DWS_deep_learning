...
pthread_t serial_pthread_id, dws_pthread_id, vibrate_motor_pthread_id;
...
#if 0
	if(gpio_init(CAMERA_LED_GPIO_INDEX, 1) < 0)
	{
		printf("initial gpio%d error!\n", CAMERA_LED_GPIO_INDEX);
		return -1;
	}

	/* pull up gpio for camera led, turn on camera led */
	gpio_write(CAMERA_LED_GPIO_INDEX, 1);
#endif
