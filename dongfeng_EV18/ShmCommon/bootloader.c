 ...
 *                0: normal; -1: unnormal
 */
static int __bootloader_logic_init(BootloaderBusinessLogic *bootloader_logic)
{
...
int bootloader_logic_init()
{
	return __bootloader_logic_init(&JMC_bootloader_logic);
}
		...
		unsigned char* reply_mesg, unsigned short* reply_mesg_len)
{
	...
	if((0x31 == *can_mesg) && (0x01 == *(can_mesg+1)) && \
			(0xF0 == *(can_mesg+2)) && (0x02 == *(can_mesg+3)))
	{
		*reply_mesg = 0x71;
		*(reply_mesg+1) = 0x01;
		*(reply_mesg+2) = 0xF0;
	...
	if(DownloadDriver == bootloader_logic->bootloader_subseq)
	{
		...
		if(0x34 == *can_mesg)
		{
			...
			if((driver_logicblock_node->logic_block_data.mem_addr != 0x00000001) || \
		...
		{
			DEBUG_INFO(enter driver data download\n);
			...
			else
			{
				...
				DEBUG_LINE();
				DEBUG_INFO(logic_block_data.crc32_cal: %8X\n, driver_logicblock_node->logic_block_data.crc32_cal);
				DEBUG_INFO(ptr_datasegment->segment_len: %4X\n, ptr_datasegment->segment_len);
				DEBUG_INFO(mesg_len-11: %d\n, mesg_len-2);
		...
		/* check data integrity */
				...
				(0xF0 == *(can_mesg+2)) && (0x01 == *(can_mesg+3)))
		{
			driver_logicblock_node = \
					list_entry(bootloader_logic->driver_list_head.prev, LogicBlockNode, logic_block_list);
		...
		unsigned char* reply_mesg, unsigned short* reply_mesg_len)
{
	...
	if(DownloadApplication == bootloader_logic->bootloader_subseq)
	{
		...
		if((0x2E == *can_mesg) && (0xF1 == *(can_mesg+1)) && (0xA0 == *(can_mesg+2)))
		{
			bootloader_logic->bootloader_subseq = DownloadApplication;
...
static int __bootloader_main_process(BootloaderBusinessLogic *bootloader_logic, \
		const unsigned char* can_mesg, unsigned short mesg_len, \
		unsigned char* reply_mesg, unsigned short* reply_mesg_len)
{
...
int bootloader_main_process(const unsigned char* can_mesg, unsigned short mesg_len, \
		unsigned char* reply_mesg, unsigned short* reply_mesg_len)
{
	return __bootloader_main_process(&JMC_bootloader_logic, can_mesg, mesg_len,\
			reply_mesg, reply_mesg_len);
}
