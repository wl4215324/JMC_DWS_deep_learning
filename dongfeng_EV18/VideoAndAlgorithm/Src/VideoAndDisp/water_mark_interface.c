...
static int getWordIndex(char *data, int len)
{
    if (len == 1)
    {
        ...
        else if (*data >= 'a' && *data <= 'z') return (*data - 0x61);
...
void *initialwaterMark(int wm_height)
{
    ...
    ALOGI("<initialwaterMark>water mark initialize, height=%d", wm_height);
    ...
    for(i = 0; i < watermark_pic_num; ++i)
    {
        ...
        wm_ctrl->wminfo.single_pic[i].y = (unsigned char*)malloc(wm_ctrl->wminfo.single_pic[i].width * \
        		                                                 wm_ctrl->wminfo.single_pic[i].height*5/2);
        ...
        wm_ctrl->wminfo.single_pic[i].alph = wm_ctrl->wminfo.single_pic[i].y + \
        		                             wm_ctrl->wminfo.single_pic[i].width * \
        		                             wm_ctrl->wminfo.single_pic[i].height;
        wm_ctrl->wminfo.single_pic[i].c = wm_ctrl->wminfo.single_pic[i].alph + \
        		                          wm_ctrl->wminfo.single_pic[i].width * \
        		                          wm_ctrl->wminfo.single_pic[i].height;
        wm_ctrl->wminfo.single_pic[i].id = i;

        buf = (unsigned char *)malloc(wm_ctrl->wminfo.single_pic[i].width * \
        		                      wm_ctrl->wminfo.single_pic[i].height * 4);
        if (NULL == buf)
        {
            fclose(fp);
            goto ALLOC_BUF_ERR;
...
{
	WaterMarkUsrInterface *usr_watermark = (WaterMarkUsrInterface*)malloc(sizeof(WaterMarkUsrInterface));
	if(!usr_watermark)
	{
		goto init_error;
	}
	...
	if (FrameHeight > 1080)
	{
		usr_watermark->mWaterMarkCtrlRec = initialwaterMark(1080);
	}
	else
	{
		usr_watermark->mWaterMarkCtrlRec = initialwaterMark(FrameHeight);
	}

	if (NULL == usr_watermark->mWaterMarkCtrlRec)
	{
		ALOGI("Fail to initialize record water mark!");
		goto init_error;
	}

	usr_watermark->mWaterMarkCtrlPrev = initialwaterMark(540);

	if (NULL == usr_watermark->mWaterMarkCtrlPrev)
	{
		ALOGI("Fail to initialize preview water mark!");
		goto init_error;
	}

	usr_watermark->mWaterMarkMultiple = initWaterMarkMultiple();
	if(!usr_watermark->mWaterMarkMultiple)
	{
		ALOGI("Fail to initialize preview water mark!");
		goto init_error;
	}

	usr_watermark->mWaterMarkEnable = 1;
	pthread_mutex_init(&usr_watermark->mWaterMarkLock, NULL);
	return usr_watermark;

init_error:
    if(usr_watermark)
    {
    	if(usr_watermark->mWaterMarkCtrlPrev)
    	{
    		free(usr_watermark->mWaterMarkCtrlPrev);
    	}

    	if(usr_watermark->mWaterMarkCtrlRec)
    	{
    		free(usr_watermark->mWaterMarkCtrlRec);
    	}

    	if(usr_watermark->mWaterMarkMultiple)
    	{
    		free(usr_watermark->mWaterMarkMultiple);
    	}
...
void waterMarkDestroy(WaterMarkUsrInterface **usr_watermark)
{
	...
	(*usr_watermark)->mWaterMarkEnable = 0;

	if ((*usr_watermark)->mWaterMarkCtrlRec != NULL)
	{
		releaseWaterMark((*usr_watermark)->mWaterMarkCtrlRec);
		(*usr_watermark)->mWaterMarkCtrlRec = NULL;
	}

	if ((*usr_watermark)->mWaterMarkCtrlPrev != NULL)
	{
		releaseWaterMark((*usr_watermark)->mWaterMarkCtrlPrev);
		(*usr_watermark)->mWaterMarkCtrlPrev = NULL;
	}

	if ((*usr_watermark)->mWaterMarkMultiple != NULL)
	{
		releaseWaterMarkMultiple((*usr_watermark)->mWaterMarkMultiple);
		(*usr_watermark)->mWaterMarkMultiple = NULL;
	}

	pthread_mutex_unlock(&((*usr_watermark)->mWaterMarkLock));
	free(*usr_watermark);
	*usr_watermark = NULL;
}
		         ...
		         WaterMarkUsrInterface *usr_watermark)
{
	...
	if(usr_watermark)
	{
		...
		if (usr_watermark->mWaterMarkEnable && (usr_watermark->mWaterMarkCtrlRec != NULL) &&\
		   (usr_watermark->mWaterMarkMultiple != NULL))
		{
			(usr_watermark->mWaterMarkIndata).y = addrVirY;
			(usr_watermark->mWaterMarkIndata).c = addrVirY + width * height;
			(usr_watermark->mWaterMarkIndata).width = width;
			(usr_watermark->mWaterMarkIndata).height = height;
...
//			mV4l2CameraMemops.vir = (unsigned int)addrVirY;
